...
# Write the function circlesIntersect(x1, y1, r1, x2, y2, r2) that takes 6 
# numbers (ints or floats) -- x1, y1, r1, x2, y2, r2 -- that describe the 
# circle centered at (x1,y1) with radius r1, and the circle centered at 
# (x2,y2) with radius r2, and returns True if the two circles intersect 
# and False otherwise.

def circlesIntersect(x1, y1, r1, x2, y2, r2):
    radius_sum = r1 + r2
    circle_distance = distance(x1, y1, x2, y2)
    if (circle_distance <= radius_sum):
        return True
...
# values -- x, bound1, and bound2, where bound1 is not necessarily less than
# bound2. If x is between the two bounds, just return it unmodified. 
# Otherwise, if x is less than the lower bound, return the lower bound, 
# or if x is greater than the upper bound, return the upper bound.

def getInRange(x, bound1, bound2):
    large = max(bound1,bound2)
    small = min(bound1,bound2)
    if (x < small):
        return small
...
# number of eggs, and returns the smallest integer number of cartons required
# to hold that many eggs, where a carton may hold up to 12 eggs.
